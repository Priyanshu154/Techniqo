...
# if (len(browser.find_elements_by_class_name("headtop")[0].find_elements_by_tag_name("a")) != 0):
#     elems = browser.find_elements_by_class_name("headtop")[0].find_elements_by_tag_name("a")[0]
#     elems.click()

# for nif in codes:
names = []
values = []
ans = input("Press 1 to confirm")
# search_box = browser.find_element_by_id("search_str")
# time.sleep(3)
# search_box.send_keys(codes[0])
# search_box.click()
# time.sleep(3)
# suglist = browser.find_element_by_id("autosuggestlist")
# anchor = suglist.find_element_by_tag_name("a")
# anchor.click()
# standalone = browser.find_elements_by_class_name("valuation_lft")
# str = standalone[0].text.split("\n")
# sheet = wb["Sheet1"]
...
#     sheet.cell(1,6 + i).value = names[i]
# wb.save("peers.xlsx")
row_no = 2
sheet = wb["Sheet1"]
        ...
        for i in range(len(names)):
            for k in range(column_index_from_string("F"), column_index_from_string("Q") + 1):
                if (sheet.cell(1, k).value == names[i]):
                    sheet.cell(row_no, 6 + i).value = values[i]
        names = []
        values = []
        wb.save("peers.xlsx")
        row_no = row_no + 1
    except Exception as e:
        print(codes[j] + " : " + sheet.cell(row_no, 1).value)
        row_no = row_no + 1
